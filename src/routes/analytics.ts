...
import { enrollments, conceptMastery, learningEvents } from '../db/schema';
...
router.get('/class/:classId/heatmap', async (req: any, res) => {
  try {
    ...
    const studentsInClass = await db.select().from(enrollments).where(eq(enrollments.classId, classId));
    ...
    const classMasteries = allMasteries.filter(m => studentIds.includes(m.studentId));
    ...
    const heatmap = Object.keys(conceptMap).map(conceptId => ({
        conceptId,
        averageMastery: Math.round(conceptMap[conceptId].totalScore / conceptMap[conceptId].count),
