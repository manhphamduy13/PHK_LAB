...
export default function MediaLibrary() {
  ...
  const getIcon = (mimeType: string) => {
    if (mimeType?.includes('video')) return <FileVideo className="w-8 h-8 text-red-500" />;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => document.getElementById('upload-input')?.click()} className="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white font-black rounded-2xl hover:bg-blue-600 transition-all border-b-4 border-blue-700 active:border-b-0 active:translate-y-1">
          <Upload className="w-5 h-5" /> Tải lên
          <input type="file" id="upload-input" className="hidden" onChange={async (e) => {
            if (e.target.files && e.target.files[0]) {
              const formData = new FormData();
              formData.append('file', e.target.files[0]);
              try {
                await fetch('/api/ai/upload-pdf', {
                  method: 'POST',
                  headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
                  body: formData
                });
            ...
            {media.map(file => (
              <div key={file.id} className="group bg-slate-50 border-2 border-slate-100 rounded-2xl p-4 hover:border-blue-300 transition-colors relative">
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                   <button onClick={() => handleDownload(file.id, file.originalName)} className="p-1.5 bg-white text-slate-400 hover:text-blue-500 rounded-lg shadow-sm"><Download className="w-4 h-4" /></button>
                   <button onClick={() => handleDelete(file.id)} className="p-1.5 bg-white text-slate-400 hover:text-red-500 rounded-lg shadow-sm"><Trash2 className="w-4 h-4" /></button>
                </div>
                <div className="h-32 flex items-center justify-center bg-white rounded-xl mb-4 border-2 border-slate-100">
                  {getIcon(file.mimeType)}
                </div>
                <p className="font-bold text-slate-900 truncate" title={file.originalName}>{file.originalName}</p>
                <div className="flex items-center justify-between mt-2">
                  <span className="text-xs font-bold text-slate-400 uppercase">{formatSize(file.size)}</span>
                </div>
              </div>
            ))}
            {media.length === 0 && (
              <div className="col-span-full text-center p-8 text-slate-500 font-bold">Chưa có tài liệu nào</div>
            )}
          </div>
        )}
      </div>
