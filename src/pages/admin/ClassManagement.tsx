...
export default function ClassManagement() {
  ...
  const handleCreateClass = async () => {
    const name = window.prompt("Tên lớp mới:");
    if (!name?.trim()) return;
    const gradeId = window.prompt("Mã khối (ví dụ: grade10):", "grade10");
    try {
      ...
      console.error(error);
      alert("Không thể tạo lớp. Vui lòng kiểm tra quyền và dữ liệu khối.");
    }
  };
    ...
    async function loadHeatmap() {
      ...
      try {
        const res = await fetch(
          `/api/analytics/class/${selectedClass}/heatmap`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-2">
            Theo dõi và giao bài tập
          </p>
        </div>
        <button
          onClick={handleCreateClass}
          className="flex items-center gap-2 bg-blue-500 text-white px-6 py-3 rounded-xl border-b-4 border-blue-700 hover:translate-y-px hover:border-b-2 transition-all font-bold"
        >
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {heatmap.map((h, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border-2 border-slate-100"
                  >
                    <div>
