...
import { FileUp, Search, CheckCircle2, AlertCircle, Loader2, Play } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">AI Pipeline Dashboard</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded-3xl border-2 border-slate-200 p-6 shadow-sm">
          <h2 className="font-black text-lg text-slate-900 mb-4">1. Tải lên PDF</h2>
          ...
          <div className="border-2 border-dashed border-slate-300 rounded-2xl p-8 text-center hover:bg-slate-50 transition-colors">
            ...
            <label htmlFor="pdf-upload" className="cursor-pointer flex flex-col items-center">
              ...
              <p className="text-sm text-slate-500 font-medium mt-1">Dung lượng tối đa: 10MB</p>
            </label>
          </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border-2 border-slate-200 p-6 shadow-sm">
          <h2 className="font-black text-lg text-slate-900 mb-4">2. Tiến trình xử lý (AI Jobs)</h2>
          ...
          <div className="space-y-4">
            ...
            {jobs.map((job) => (
              <div key={job.id} className="border-2 border-slate-100 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <p className="font-bold text-slate-800">Job: {job.id.slice(0,8)}... - {job.task}</p>
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Trạng thái: <span className="font-bold text-blue-600">{job.status}</span>
                  </p>
                  {job.error && <p className="text-xs text-red-500 mt-1">{job.error}</p>}
                </div>
                <div>
                  {job.status === 'PROCESSING' && <Loader2 className="w-6 h-6 text-blue-500 animate-spin" />}
                  {job.status === 'COMPLETED' && (
                    <div className="flex gap-2">
                       <CheckCircle2 className="w-6 h-6 text-emerald-500" />
                       <button 
                          onClick={() => {
                            const res = job.resultData ? JSON.parse(job.resultData) : null;
                            if (res && res.lessonId) {
                              navigate(`/admin/ai-content/${res.lessonId}/review`);
                            }
                          }}
